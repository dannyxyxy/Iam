<script>  
  export default {
  name: 'App',
  data(){//데이터 변수값 저장
    return{
      PostMain: "/PostMain",
      isDarkMode:false,
      i:3,
      title:"Title",
      summary:"summary",
      user:"danny",
      likeCount:0,
      category1: "여행",
      category1_1: "총 120개의 포스트",
      category2: "YOLO",
      category2_1: "총 100개의 포스트",
    }
  },
  methods:{//함수만드는곳 this. 꼭 붙힐것
    toggleMode(){
      this.isDarkMode=!this.isDarkMode;
    },
  },

  components: {
  }
}
</script>

<template>
  <div class="maintext" :class="{ 'dark-mode': isDarkMode }">
    <div>
      <div class="button-container">
      <button class="btn" :class="{ active: selected === 'popular' }" @click="selected = 'popular'">
        <img src="../assets/img/trend.png">카테고리 인기순
      </button>
    </div>
    <div class="button-container2">
      <!-- 추후 카테고리로 라우팅하거나 a 태그로 링크 활용가능 -->
      <button>{{category1}}</button>
      <button>{{category1_1}}</button>
    </div>
    <div class="container">
      <div class="box" v-for="n in i" :key="n">
        <router-link :to="PostMain">
        <img src="../assets/img/hi.jpg">
        </router-link>
        <div class="box-summary">
          <div>
            <h3>{{title}}</h3><h5>{{summary}}</h5>
          </div>
          <div class="circle"><img src="../assets/img/profile.jpg"></div>
          <div class="username">by <span style="font-weight:bold">{{user}}</span></div>
          <div>
            <button class="like-btn" @click="likeCount++"><i class="fas fa-heart"></i><img src="../assets/img/heart.png">{{likeCount}}</button>
          </div>
        </div>
      </div>
    </div>
    <div class="button-container2">
      <!-- 추후 카테고리로 라우팅하거나 a 태그로 링크 활용가능 -->
      <button>{{category2}}</button>
      <button>{{category2_1}}</button>
    </div>
    <div class="container">
      <div class="box" v-for="n in i" :key="n">
        <router-link :to="PostMain">
        <img src="../assets/img/hi.jpg">
        </router-link>
        <div class="box-summary">
          <div>
            <h3>{{title}}</h3><h5>{{summary}}</h5>
          </div>
          <div class="circle"><img src="../assets/img/profile.jpg"></div>
          <div class="username">by <span style="font-weight:bold">{{user}}</span></div>
          <div>
            <button class="like-btn" @click="likeCount++"><i class="fas fa-heart"></i><img src="../assets/img/heart.png">{{likeCount}}</button>
          </div>
        </div>
      </div>
    </div>
  </div>
  </div>
</template>
